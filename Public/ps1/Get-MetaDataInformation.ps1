...
Function Get-MetaDataInformation{
    ...
    foreach ($namespace in $namespacesArray) {
        if ($serviceAccount -eq "Myself"){
            if(Get-CanIExecuteInNamespace -namespace $namespace -accesstoken $accesstoken -command "get pods --subresource=exec"){
                ...
                foreach($pod in $runningPod){
                    ...
                    $curl = Perform-KubectlExecCommand -pod $pod -namespace $namespace -accesstoken $accesstoken -command 'which curl'
                    ...
                    $metadataData = Perform-KubectlExecCommand -pod $pod -namespace $namespace -accesstoken $accesstoken -command 'curl "http://169.254.169.254/metadata/instance?api-version=2020-10-01" -H Metadata:true -s 2>$null'
